...
start_on_device() {
    ...
    # Check if Multi-LinUCB solver exists
    if ! adb -s "$DEVICE_SERIAL" shell "test -f /data/local/tmp/multilin && echo exists" | grep -q "exists"; then
        echo "⚠️  Warning: Multi-LinUCB solver not found on Device $DEVICE_NUM"
        echo "   Run: cd device_scripts && ./deploy_multilin.sh"
        return 1
    fi
    ...
    if [ -n "$BID_RUNNING" ] && [ -n "$FEEDBACK_RUNNING" ]; then
        echo "✓ Multi-LinUCB bid & feedback listeners started on Device $DEVICE_NUM"
...
echo "========================================="
echo "Multi-LinUCB Bid & Feedback Listeners Started"
echo "========================================="
echo ""
