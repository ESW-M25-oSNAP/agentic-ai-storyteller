...
for i in ${!DEVICES[@]}; do
    ...
    if [ -z "$IP" ]; then
        IP=$(adb -s $SERIAL shell "ip addr show swlan 2>/dev/null | grep 'inet ' | awk '{print \$2}' | cut -d'/' -f1" | tr -d '\r\n')
        if [ -z "$IP" ]; then
            IP="0.0.0.0"
        fi
    else
    ...
    adb -s $SERIAL push termux/mesh_listener.sh /data/local/tmp/mesh/mesh_listener.sh >/dev/null 2>&1
    adb -s $SERIAL push termux/mesh_sender.sh /data/local/tmp/mesh/mesh_sender.sh >/dev/null 2>&1
